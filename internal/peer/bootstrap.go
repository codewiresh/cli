...
func DialEnvironmentPeerTCP(ctx context.Context, client *platform.Client, orgID, envID string, port uint16) (net.Conn, *tailnetlib.Conn, error) {
	...
	go func() {
		...
		for {
			// Read from current coordinator connection.
			for {
			...
			debugf("coordinator reconnecting in %s...", backoff)
			select {
			...
			newWS, _, err := websocket.Dial(ctx, wsURL, wsDialOpts)
			if err != nil {
				debugf("coordinator reconnect failed: %v", err)
				backoff = min(backoff*2, 5*time.Second)
				continue
			}

			// Swap connection under lock so the node callback writes to the new one.
			wsMu.Lock()
			old := wsConn
			wsConn = newWS
			node := lastNode
			...
			debugf("coordinator reconnected")

			// Re-send current node info so the server learns our identity.
			if node != nil {
				data, _ := json.Marshal(coordinateMsg{Type: "node", Node: node})
				wsMu.Lock()
				_ = wsConn.Write(ctx, websocket.MessageText, data)
				wsMu.Unlock()
			}
